...
local ban_remote = game.ReplicatedStorage.Remotes:WaitForChild("\208\149rrrorLog")
ban_remote:Destroy()
...
OpenClosestDoor = Miscellaneous.Button({
	...
	Callback = function()
		...
		for _, door in pairs(workspace:GetChildren()) do
			if door:IsA("Model") then
				if door:GetAttribute("KeyDoor") and door:FindFirstChild("Main") then
					if (door:GetPivot().Position - game.Players.LocalPlayer.Character:GetPivot().Position).Magnitude <= last then
						last = (door:GetPivot().Position - game.Players.LocalPlayer.Character:GetPivot().Position).Magnitude
						closest = door
					end
				end
			end
		...
		if closest then
			local unit = (closest.Main.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Unit

			for i = 1,20 do
				game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(closest.Main.Position + unit * 3)
