...
if game.PlaceId == 292439477 then
    for _,v in pairs (game:GetService("Workspace").Players:GetChildren()) do
        if v:IsA("Folder") then
            for _,Player in pairs (v:GetChildren()) do
                Ut.Add(Player)
            end
        end
    end
...
local library = {
    ...
    images = {
        ['gradientp90'] = 'https://dodging.net/rezkoclient/lua/gradientp90.png';
        ['gradientp45'] = 'https://dodging.net/rezkoclient/lua/gradientP45.png';
        ['colorhue'] = 'https://dodging.net/rezkoclient/lua/hue.png';
        ['colorsat1'] = 'https://dodging.net/rezkoclient/lua/colorsat1';
        ['colorsat2'] = 'https://dodging.net/rezkoclient/lua/colorsat2';
        ['colortrans'] = 'https://dodging.net/rezkoclient/lua/transparency.png';
    };
...
library.gamename = game.PlaceId == 7353845952 and "project delta" or game.PlaceId == 4991214437 and "town" or game.PlaceId == 6652350934 and "town" or "universal"
...
function library:init()
    ...
    function self.NewWindow(data)
        ...
        window.title = game.PlaceId == 7353845952 and "Alsike - Project Delta" or game.PlaceId == 4991214437 and "Alsike - Town" or game.PlaceId == 6652350934 and "Alsike - Town" or "Alsike - Universal"
...
if game.PlaceId == 7353845952 then
    function library:CreateSettingsTab(menu)
        ...
        --[[if game.PlaceId == 4991214437 then
            mainSection:AddToggle({text = 'Join New Game On Kick', flag = 'votekickrejoin'})
        end]]
    ...
    local function esprend() 
        ...
        for _,v in next, library.flags.espexitd do
            if v == "Distance" then
                niggad = true
            end
            if v == "Name" then
        ...
        for _,v in pairs(game:GetService("Workspace").NoCollision.ExitLocations:GetChildren()) do
            if v:IsA("Part") then
                Utility.Add(v)
            end
        end
        ...
        for _,v in pairs(game:GetService("Workspace").NoCollision.ExitLocations:GetChildren()) do
            if v.ClassName == ("Part") then
                ...
                for _,Drawing in pairs (PLRD) do
                    Drawing.Visible = false
                end
                
                local Pos, OnScreen = Camera:WorldToViewportPoint(v.Position)
                if OnScreen then
                    local IhateMyLifeSize = (Camera:WorldToViewportPoint(v.Position - Vector3.new(0, 3, 0)).Y - Camera:WorldToViewportPoint(v.Position + Vector3.new(0, 2.6, 0)).Y) / 2
                    local BoxSize = Vector2.new(math.floor(IhateMyLifeSize * 1), math.floor(IhateMyLifeSize * 1.9))
                    local BoxPos = Vector2.new(math.floor(Pos.X - IhateMyLifeSize * 1 / 2), math.floor(Pos.Y - IhateMyLifeSize * 1.6 / 2))

                    local DistanceFromCharacter = tostring(math.ceil(LocalPlayer:DistanceFromCharacter(v.Position) / 5)).. "m"

                    local Exit = PLRD.Exit
                    
                    if library.flags.exittog then
                        Exit.Visible = true
                        Exit.Color = library.flags.exittogcol
                        Exit.Font = library.flags.espfont
                        Exit.Size = library.flags.espsize
                        Exit.Outline = library.flags.espout
                        Exit.Position = BoxPos
                        if niggan and niggad then
                            Exit.Text = "Exit\n" ..DistanceFromCharacter
                        elseif niggan then
                            Exit.Text = "Exit"
                        elseif niggad then
                            Exit.Text = DistanceFromCharacter
                        end
                    end
                end
            end
            ...
            if library.flags.espenabled and PLRDS[Player] and isTarget(Player, library.flags.espteam) then
                ...
                if not OnScreen then
                    ...
                    if library.flags.tooltog then
                        if ptool ~= nil then
                            ...
                            if library.flags.espbold then
                                ...
                                if library.flags.tt and tname == "["..Player.Name.."]" then
                                    ToolB.Color = library.flags.tcol
                                else
                                    ToolB.Color = library.flags.tooltogcol
                                end
                                ToolB.Font = library.flags.espfont
                                ToolB.Size = library.flags.espsize
                                ToolB.Visible = true
                                ToolB.Outline = false
                            end
                            Y_off += 15
                    ...
                    if library.flags.nametog then
                        ...
                        if library.flags.espbold then
                            if library.flags.espcase == "Normal" then
                                NameB.Text = Player.Name
                            elseif library.flags.espcase == "lowercase" then
                                NameB.Text = string.lower(Player.Name)
                            elseif library.flags.espcase == "UPPERCASE" then
                                NameB.Text = string.upper(Player.Name)
                            end
                            NameB.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X + 1, BoxPos.Y - 6 - Name.TextBounds.Y)
                            if library.flags.tt and tname == "["..Player.Name.."]" then
                                NameB.Color = library.flags.tcol
                            else
                                NameB.Color = library.flags.nametogcol
                            end
                            NameB.Font = library.flags.espfont
                            NameB.Size = library.flags.espsize
                            NameB.Visible = true
                            NameB.Outline = false
                        end
                    end
                    ...
                    if library.flags.distancetog then
                        ...
                        if library.flags.espbold then
                            ...
                            if library.flags.tt and tname == "["..Player.Name.."]" then
                                DistanceB.Color = library.flags.tcol
                            else
                                DistanceB.Color = library.flags.distancetogcol
                            end
                            DistanceB.Font = library.flags.espfont
                            DistanceB.Size = library.flags.espsize
    ...
    local function movement()
        ...
        if library.flags.mvfly then
            ...
            if library.flags.mvflytype == "Noclip" and library.flags.mvflykey then
                for _,v in pairs(LocalPlayer.Character:GetDescendants()) do
                    if v:IsA("BasePart") and v.CanCollide == true then
                        v.CanCollide = false
                    end
            ...
            if library.flags.mvflykey then
    ...
    local function visfuncs()
        ...
        --[[if game.PlaceId == 4991214437 then
            for _,v in pairs(Workspace.Camera:GetChildren()) do
                if v.Name == "BulletHole" or v.Name == "Shells" then
                    v:Destroy()
                end
            end

            if library.flags.votekickrejoin then
                if game:GetService("Players")[LocalPlayer.Name].PlayerGui:FindFirstChild("VoteKickGui") then
                    if game:GetService("Players")[LocalPlayer.Name].PlayerGui.VoteKickGui.Main.Speech.Text == "Kick " ..LocalPlayer.Name.. "?" then
                        if game:GetService("Players")[LocalPlayer.Name].PlayerGui.VoteKickGui.Main.VoteTally.Text == "7 / 8" then
                            local Servers = game.HttpService:JSONDecode(game:HttpGet(("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=Asc&limit=100"):format(game.PlaceId)))
    ...
    function menu()
        ...
        local chamesp = playerSec:AddToggle({text = "Chams", tooltip = "This is not streamproof!", flag = "chametog"})
        ...
        playerSec:AddToggle({text = "Bold Text", flag = "espbold"})
...
elseif game.PlaceId == 4991214437 or game.PlaceId == 6652350934 then
    ...
    local function esprend() 
        ...
        for _,v in pairs(game.Players.LocalPlayer.Character:GetChildren()) do
            local toolthing = v:FindFirstChildWhichIsA("Tool")

            if not toolthing then
                for _,g in pairs(v:GetChildren()) do
                    if g:IsA("BasePart") then
                        if g:FindFirstChildWhichIsA("Highlight") ~= nil then
                            g.Highlight:Destroy()
                        end
                    end
                end
            ...
            if v:IsA("Tool") then
                for _,g in pairs(v:GetChildren()) do
                    if g:IsA("BasePart") then
                           if library.flags.gunchams then
                            if g:FindFirstChildWhichIsA("Highlight") == nil then
                                local highlight = Instance.new("Highlight", g)
                            end
                            g.Highlight.FillColor = library.flags.gunchamscol
                            g.Highlight.OutlineTransparency = library.flags.gunchamstransb
                            g.Highlight.FillTransparency = library.flags.gunchamstrans
                            g.Highlight.OutlineColor = library.flags.gunchamscolb
                            g.Highlight.DepthMode = Enum.HighlightDepthMode.Occluded
                        else
                            if g:FindFirstChildWhichIsA("Highlight") ~= nil then
                                g.Highlight:Destroy()
                            end
    ...
    function menu()
        ...
        local gunSec = combatTab:AddSection("Gun Mods", 1);
        gunSec:AddButton({text = 'Enable Gun Mods', confirm = true, callback = function()
                ...
                for _, f in pairs(debug.getregistry()) do
                    if typeof(f) == "function" and not is_synapse_function(f) then
                        ...
                        if typeof(script) == "Instance" and script.Name == "GunScript" then
                            ...
                            t.Projectiles = 800
                            t.AimScatterMultiplyer = 1.7
                            t.ReloadSpeed = 0.00001
                            t.GunRecoil = 0.10
        ...
        local gunCt = lcSec:AddToggle({text = "Gun Chams", flag = "gunchams"})
        gunCt:AddColor({text = "Fill Color", flag = "gunchamscol"})
        gunCt:AddColor({text = "Outline Color", flag = "gunchamscolb"})
        gunCt:AddSlider({text = "", flag = "gunchamstrans", value = 0.5, min = 0, max = 1, increment = 0.01})
        gunCt:AddSlider({text = "", flag = "gunchamstransb", value = 0.5, min = 0, max = 1, increment = 0.01})
...
--[[elseif game.PlaceId == 292439477 then
    ...
    local function esprend() 
        ...
        for _,v in pairs (game:GetService("Workspace").Players:GetChildren()) do
            if v:IsA("Folder") then
                for _,g in pairs (v:GetChildren()) do
                    if library.flags.chametog then
                        if g:FindFirstChildWhichIsA("Highlight") == nil then
                            local highlight = Instance.new("Highlight", g)
                            highlight.FillTransparency = 0.4
                            highlight.OutlineTransparency = 1
                        end
                        g.Highlight.FillColor = library.flags.chamtogcol
                        g.Highlight.OutlineTransparency = library.flags.chamstransb
                        g.Highlight.FillTransparency = library.flags.chamstrans
                        g.Highlight.OutlineColor = library.flags.chamstolcolo
                    else
                        if g:FindFirstChildWhichIsA("Highlight") then
                            g:FindFirstChildWhichIsA("Highlight"):Destroy()
                        end
                    end
                end
            end
        ...
        for _,v in pairs (game:GetService("Workspace").Players:GetChildren()) do
            if v:IsA("Folder") then
                for _,Player in pairs (v:GetChildren()) do
                    if library.flags.espenabled and PLRDS[Player] then
                        local PLRD = PLRDS[Player]
                        for _,v in pairs (PLRD) do
                            v.Visible = false
                        end

                        local Character = Player:GetChildren()

                        local poser = Camera:WorldToViewportPoint(Character.Head.Position)
                        local fov = library.flags.aimfov
                        local mousePos = Vector3.new(UserInputService:GetMouseLocation().x, UserInputService:GetMouseLocation().y, 0)
                        local DistanceFromCharacter = tostring(math.ceil(LocalPlayer:DistanceFromCharacter(RootPart.Position) / 5)).. "m"
                        local alivecheck = true
                        ...
                        if not OnScreen then
                            local niggars = Character.PrimaryPart
                            local proj = Camera.CFrame:PointToObjectSpace(Character.Head.Position)
                            local ang = math.atan2(proj.Z, proj.X)
                            local dir = Vector2.new(math.cos(ang), math.sin(ang))
                            local pos = (dir * library.flags.espard * .5) + Camera.ViewportSize / 2
                            local Drawing = PLRD.Offscreen
                            ...
                            local IhateMyLifeSize = (Camera:WorldToViewportPoint(Character.Head.Position - Vector3.new(0, 3, 0)).Y - Camera:WorldToViewportPoint(Character.Head.Position + Vector3.new(0, 2.6, 0)).Y) / 2
                            local BoxSize = Vector2.new(math.floor(IhateMyLifeSize * 1.1), math.floor(IhateMyLifeSize * 2))
                            local BoxPos = Vector2.new(math.floor(Pos.X - IhateMyLifeSize * 1 / 2), math.floor(Pos.Y - IhateMyLifeSize * 1.6 / 2))
                            local Name = PLRD.Name
                            local Distance = PLRD.Distance
