...
[name] get better]])
    ...
    if not isfolder("shambles haxx/" .. menu.game) then
        makefolder("shambles haxx/" .. menu.game)
    end

    if not isfile("shambles haxx/" .. menu.game .. "/Default.bb") then
        writefile("shambles haxx/" .. menu.game .. "/Default.bb", "")
    end
end
...
local function GetConfigs()
	local result = {}
	local directory = "shambles haxx\\" .. menu.game
...
function menu.Initialize(menutable)
	...
	do
		...
		Draw:MenuBigText(MenuName or "shambles haxx", true, false, 6, 6, bbmenu)
	...
	function menu.saveconfig()
		...
		writefile("shambles haxx/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb", figgy)
		CreateNotification('Saved "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.bb"!')
		UpdateConfigs()
	end

	function menu.loadconfig()
		local configname = "shambles haxx/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb"
	...
	local function buttonpressed(bp)
		...
		elseif bp == menu.options["Settings"]["Configuration"]["Delete Config"] then
			delfile("shambles haxx/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb")
		...
		if menu then
			...
			if menu.open then
				if menu.tabnames[menu.activetab] == "Settings" then
					local menutext = menu:GetVal("Settings", "Cheat Settings", "Custom Menu Name") and menu:GetVal("Settings", "Cheat Settings", "MenuName") or "shambles haxx"
...
menu.Initialize({
	...
	},
    {
        ...
        content = {
            ...
            {
                name = {"Weapon Tweaks", "Game Tweaks"},
                autopos = "left",
                autofill = true,
                size = 100,
                [1] = {
					content = {
						{
							...
							value = false,
							unsafe = true,
							tooltip = "Allows shambles haxx to modify weapons.",
						},
						{
							type = TOGGLE,
							name = "No Spread",
					...
					content = {
						{
							...
							value = false,
							unsafe = true,
							tooltip = "Allows shambles haxx to modify game.",
						},
						{
							type = TOGGLE,
							name = "Infinite Cash",
                ...
                [1] = {
                    content = {
                        ...
                        },
						{
							...
							name = "Primary",
							value = 1,
							values = {"MP9", "MAC10", "MP7", "UMP", "P90", "Bizon", "M4A4", "M4A1", "AK47", "Famas", "Galil", "AUG", "SG", "AWP", "Scout", "G3SG1", "XM", "Nova", "Sawed off", "Mag 7"},
						},
						{
							type = DROPBOX,
							name = "Secondary",
							value = 1,
							values = {"USP", "P2000", "Glock", "DualBerettas", "P250", "FiveSeven", "Tec9", "CZ", "DesertEagle", "R8"},
						},
						{
							type = DROPBOX,
							name = "Chat Spam",
                ...
                [2] = {
                    content = {
                        ...
                        },
						{
							...
							name = "Kill Sound Id",
							value = 1,
							values = {"Neverlose", "Skeet", "Rust", "Baimware", "Oni-Chan", "Rifk7"},
						},
                        {
                            type = SLIDER,
...
Health: %s
	...
	--Draw:Circle(false, 0, 0, 0, 1, 32, { 0, 0, 0, 0 }, allesp[1][2])
	...
	local function rvec(v, r)
		local a = math.cos(r)
		local b = math.sin(r)
		return Vector2.new(a * v.X - b * v.Y, b * v.X + a * v.Y)
	end
	...
	function ESP()
		for _,Player in pairs (Players:GetPlayers()) do
			if menu:GetVal("Players", "Enemy ESP", "Enabled") and PLRDS[Player] and isTarget(Player, false) and isAlive(Player) then
				...
				local Pos, OnScreen = Camera:WorldToViewportPoint(RootPart.Position)
				--local transp = 1 - (menu:GetVal("Players", "Enemy ESP", "Enemy Chams Inline", COLOR2)[4]/255)
				...
				if not OnScreen then
					local ptbs = Camera.CFrame:PointToObjectSpace(RootPart.Position)
					local ang = math.atan2(ptbs.x, ptbs.y)
					local dir = Vector2.new(math.cos(ang), math.sin(ang))
					local pos = (dir * (menu:GetVal("Players", "Enemy ESP", "Arrow Distance") * 10) * .5) + Camera.ViewportSize / 2
					local OutOfView = PLRD.Offscreen

					
					if menu:GetVal("Players", "Enemy ESP", "Out of View") then
						OutOfView.PointA = pos
						OutOfView.PointB = pos - rvec(dir, math.rad(35)) * menu:GetVal("Players", "Enemy ESP", "Arrow Size")
						OutOfView.PointC = pos - rvec(dir, -math.rad(35)) * menu:GetVal("Players", "Enemy ESP", "Arrow Size")
						OutOfView.Color = menu:GetVal("Players", "Enemy ESP", "Out of View", COLOR, true)
						OutOfView.Filled = true
						OutOfView.Transparency = menu:GetVal("Players", "Enemy ESP", "Out of View", COLOR)[4] / 255
						OutOfView.Visible = true

						if menu:GetVal("Players", "Enemy ESP", "Arrow Types")[1] then
							
							local color = menu:GetVal("Players", "ESP Settings", "Highlight Friends") and table.find(menu.friends, Player.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Friends", COLOR, true) or menu:GetVal("Players", "ESP Settings", "Highlight Priority") and table.find(menu.priority, Player.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Priority", COLOR, true) or menu:GetVal("Players", "Enemy ESP", "Name", COLOR, true)
							...
							Name.Position = pos - rvec(dir, math.rad(50))
							Name.Color = color
							Name.Transparency = menu:GetVal("Players", "Enemy ESP", "Name", COLOR)[4] / 255
					...
					if menu:GetVal("Players", "Enemy ESP", "Name") then
						if menu:GetVal("Players", "ESP Settings", "Text Case") == 2 then
							Name.Text = Player.DisplayName
						elseif menu:GetVal("Players", "ESP Settings", "Text Case") == 1 then
							Name.Text = string.lower(Player.DisplayName)
	...
	menu.connections.renderstepped_cb = game.RunService.RenderStepped:Connect(function()
		do
			LegitRage()
		end
		do
...
do
	...
	wm.width = #fulltext * 7 + 13
	wm.height = 19
	wm.rect = {}

	local ihatemylife = {}

	--local imagelololololo = readfile("shambles haxx/" ..menu:GetVal("Visuals", "Interface", "Icon Name"))
	...
	Draw:OutlinedRect(false, wm.pos.x - 1, wm.pos.y - 1, wm.width + 2 + 20, wm.height + 2, { 0, 0, 0, 255 * 0.4 }, wm.rect)
	Draw:OutlinedText(
		fulltext,
		2,
		false,
...
CreateNotification("Press RIGHTSHIFT to open and close the menu!")
CreateThread(function()
