...
if not BBOT then
	BBOT = { username = "aitech stable build" }
end
local menu

local MenuName = isfile("aitech/menuname.txt") and readfile("aitech/menuname.txt") or nil
...
if not isfolder("aitech") then
	makefolder("aitech")
end

do -- Files
	if not isfile("aitech/relations.at") then
		writefile("aitech/relations.at", "friends:\npriority:")
	end

    if not isfile("aitech/chatspam.at") then
        writefile("aitech/chatspam.at",
            [[BEACH BOAT ON TOP
AI TECH NEW UPDATE 2058 ةشساشممشا
ؤقهحلاخف خهى فخح غثشا ةغ لاقخ
aitech does not miss]])
    end

    if not isfile("aitech/killsay.at") then
        writefile("aitech/killsay.at",
...
[name] get better]])
    ...
    if not isfolder("aitech/" .. menu.game) then
        makefolder("aitech/" .. menu.game)
    end

    if not isfile("aitech/" .. menu.game .. "/default.at") then
        writefile("aitech/" .. menu.game .. "/default.at", "")
    end
end

do
    local customtxt = readfile("aitech/chatspam.at")
    for s in customtxt:gmatch("[^\n]+") do
        table.insert(customChatSpam, s)
    end

    customtxt = readfile("aitech/killsay.at")
    for s in customtxt:gmatch("[^\n]+") do
...
local function UnpackRelations()
	local str = isfile("aitech/relations.at") and readfile("aitech/relations.at") or nil
	local final = {
		friends = {},
		priority = {},
	}
...
local function WriteRelations()
	...
	writefile("aitech/relations.at", str)
end
CreateThread(function()
...
function menu.Initialize(menutable)
	...
	do
		...
		Draw:MenuBigText(MenuName or "aitech.pub", true, false, 6, 6, bbmenu)
	...
	function menu.saveconfig()
		...
		writefile("aitech/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".at", figgy)
		CreateNotification('Saved "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.at"!')
		UpdateConfigs()
	end

	function menu.loadconfig()
		local configname = "aitech/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".at"
		if not isfile(configname) then
			CreateNotification('"'.. menu.options["Settings"]["Configuration"]["ConfigName"][1].. '.at" is not a valid config.')
			return
		end
	...
	local function buttonpressed(bp)
		...
		elseif bp == menu.options["Settings"]["Configuration"]["Delete Config"] then
			delfile("intellect/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb")
		...
		if menu then
			...
			if menu.open then
				if menu.tabnames[menu.activetab] == "Settings" then
					local menutext = menu:GetVal("Settings", "Cheat Settings", "Custom Menu Name") and menu:GetVal("Settings", "Cheat Settings", "MenuName") or "aitech.pub"
							...
							content = {
								{
									...
									name = "Enabled",
									value = false,
									tooltip = "Allows intellect to modify weapons.",
								},
								{
									type = SLIDER,
							...
							[1] = {
								content = {
									...
									},
									{
										...
										tooltip = "Spams chat, Custom options are located in the aitech/chatspam.at",
									},
									{
										type = TOGGLE,
...
CreateNotification(string.format("finished loading the " .. menu.game .. " cheat. (%d ms)", menu.load_time))
